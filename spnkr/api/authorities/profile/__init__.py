...
class ProfileAuthority(BaseAuthority):
    ...
    def get_profiles(
        self, xuids: list[str], settings: list[ProfileSetting] | None = None
        ...
        return models.ProfileResponse.from_dict(resp.json())

    def get_profile_by_xuid(
        self, xuid: str, settings: list[ProfileSetting] | None = None
    ) -> models.ProfileResponse:
        ...
        resp = self._session.get(
            url, params=params, auth_method=self.AUTH_METHOD
        )
        if resp.status_code == 429:
            raise ApiRateLimitException("Request rate exceeded")
        resp.raise_for_status()
    ...
    def get_profile_by_gamertag(
        self, gamertag: str, settings: list[ProfileSetting] | None = None
    ) -> models.ProfileResponse:
