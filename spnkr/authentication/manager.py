...
class AuthenticationManager:
    ...
    def refresh_tokens(self) -> None:
        ...
        if self.halo_xsts_token is None or not self.halo_xsts_token.is_valid():
            self.halo_xsts_token = self.request_xsts_token(
                XSTS_V3_HALO_AUDIENCE
            )
    ...
    def _oauth2_token_request(
        ...
        return mdl.OAuth2TokenResponse.from_dict(resp.json())

    def request_user_token(
        self, relying_party: str = "http://auth.xboxlive.com"
    ) -> mdl.XAUResponse:
        ...
        resp.raise_for_status()
        return mdl.XAUResponse.from_dict(resp.json())
    ...
    def request_xsts_token(self, relying_party: str) -> mdl.XSTSResponse:
        ...
        if resp.status_code == 401:
            print(
                "Failed to authorize you! Your password or username may be wrong or you are "
                "trying to use child account (< 18 years old)"
            )
            raise AuthenticationException()
        resp.raise_for_status()
        return mdl.XSTSResponse.from_dict(resp.json())
    ...
    def request_spartan_token(self) -> mdl.SpartanTokenResponse:
        ...
        resp.raise_for_status()
        return mdl.SpartanTokenResponse.from_dict(resp.json())
    ...
    def request_clearance_token(self) -> mdl.ClearanceTokenResponse:
        ...
        resp.raise_for_status()
        return mdl.ClearanceTokenResponse.from_dict(resp.json())
